...
public class EvolutionApiClient {
    ...
    public void sendMessage(BusinessContact contact, String number, String text) {
        ...
        EvolutionTextRequest request = EvolutionTextRequest.builder()
                .number(number)
                .options(EvolutionOptions.builder()
                        .delay(1200)
                        .presence("composing")
                        .linkPreview(false)
                        .build())
                .textMessage(EvolutionTextMessage.builder()
                        .text(text)
                        .build())
                .build();

        try {
            Response<Object> response = evolutionApi.sendText(instanceId, apiKeyToUse, request).execute();
            if (!response.isSuccessful()) {
                String errorBody = response.errorBody() != null ? response.errorBody().string() : "null";
                log.error("Failed to send message via Evolution API. Code: {}, Body: {}", response.code(), errorBody);
    ...
    public CreateInstanceResponse createInstance(String instanceName, String token) {
        ...
        try {
            Response<CreateInstanceResponse> response = evolutionApi.createInstance(globalApiKey, request).execute();
            if (!response.isSuccessful()) {
                String errorBody = response.errorBody() != null ? response.errorBody().string() : "null";
                log.error("Failed to create instance. Code: {}, Body: {}", response.code(), errorBody);
                throw new RuntimeException("Evolution API create instance failed: " + response.code());
            }
    ...
    public Optional<ConnectInstanceResponse> connectInstance(String instanceName) {
        try {
            Response<ConnectInstanceResponse> response = evolutionApi.connectInstance(instanceName, globalApiKey).execute();
            ...
            String errorBody = response.errorBody() != null ? response.errorBody().string() : "null";
            log.error("Failed to connect instance '{}'. Code: {}, Body: {}", instanceName, response.code(), errorBody);
            throw new RuntimeException("Evolution API connect instance failed with code: " + response.code());
